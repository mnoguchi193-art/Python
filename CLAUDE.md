...
| `ai/` | AGI-inspired toy demos: neural net (backprop), Q-learning, A*, episodic/semantic memory, cognitive agent loop, feature-based transfer learning (multi-floor maze), model-based RL (Dyna-Q), genetic algorithm (string/knapsack/policy evolution), optimizer comparison (SGD/momentum/Adam), MCTS (tic-tac-toe), MCTS scaling study (sims vs strength, minimax reference) |
